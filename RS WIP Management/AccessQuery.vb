...
'                Dim resetCheckCmd As New OleDbCommand("SELECT COUNT(*) FROM Reset_Log WHERE Reset_Date = @ResetDate", Dbconnection)
...
'                    Dim resetCmd As New OleDbCommand("UPDATE WIPM_Process_tb SET [SAM_Out] = 0, [Target_WIP] = 0", Dbconnection)
'                    resetCmd.ExecuteNonQuery()

'                    ' Step 2: Distribute GAP values
'                    Dim selectProductsCmd As New OleDbCommand("SELECT DISTINCT [Product] FROM WIPM_Process_tb WHERE [GAP] <> 0", Dbconnection)
'                    Using productReader As OleDbDataReader = selectProductsCmd.ExecuteReader()
'                        While productReader.Read()
'                            Dim productName As String = productReader("Product").ToString()

'                            ' Get the first row for the current product with non-zero GAP
'                            Dim selectGapCmd As New OleDbCommand("SELECT TOP 1 [ID], [GAP] FROM WIPM_Process_tb WHERE [Product] = @Product AND [GAP] <> 0", Dbconnection)
'                            selectGapCmd.Parameters.AddWithValue("@Product", productName)

'                            Using gapReader As OleDbDataReader = selectGapCmd.ExecuteReader()
...
'                                    Dim updateSamOutCmd As New OleDbCommand("UPDATE WIPM_Process_tb SET [SAM_Out] = @Gap WHERE [ID] = @ID", Dbconnection)
...
'                    Dim logResetCmd As New OleDbCommand("INSERT INTO Reset_Log (Reset_Date) VALUES (@ResetDate)", Dbconnection)
...
'                    Dim updateCmd As New OleDbCommand("UPDATE WIPM_Process_tb " &
'                                                  "SET GAP = @NewGap " &
...
'                ' Create an OleDbCommand and add parameters
'                Using command As New OleDbCommand(query, Dbconnection)
